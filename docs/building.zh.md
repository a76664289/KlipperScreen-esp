...
board 取值：`cyd_2432s028r` / `e32r35t` / `esp32s3-st7789-320_240-ec11` / `esp32-st7735s-128_160-ec11` / `esp32-st7789-320_240-ec11` / `esp32s3-st7796-480_320-xpt2046-ec11` / `jc8048w550` / `esp32s3-JLC-SZP` / `esp32s3-retro-go`。每板型有独立的构建目录与 sdkconfig（芯片目标不同，不能混用）。
