/*
 * BSP: esp32s3-ILI9341-320_240-xpt2046-ec11
 *   ESP32-S3-DevKitC-1 N16R8 + 320x240 ILI9341 SPI 屏 + XPT2046 电阻触摸 + EC11 旋钮。
 *
 * 引脚与其他带旋钮+触摸的 S3 机型完全一致（esp32s3-st7789/st7796/ILI9488 同一底座接线）：
 ...
 *
 * ILI9341 面板参数沿用 CYD / esp32-ILI9341-320_240-ec11 实测值：
 *   BGR、横屏 mirror(true,true)、默认 INVOFF 即正常颜色、SPI 先发像素高字节
 *   （flush/lcd_push 就地字节交换，同 st7796 板；与 ILI9488 板相反）。
 * 触摸无真机提取的出厂默认值：touch.json 缺失/损坏即进入两点校准流程。
 * 模组方向/边缘有差异时用设置页的 180° 旋转 / 水平镜像 / 反色开关修正。
 */
#include "sdkconfig.h"
#if CONFIG_BOARD_ESP32S3_ILI9341_EC11
...
{
    /* ILI9341 走 SPI 要求先发像素高字节。就地交换→推→等 DMA 完成→交换还原，
       不拷临时副本（同 CYD / st7796 板的做法与原因） */
...
{
    /* ILI9341 默认 INVOFF 即正常颜色，"反色"开关直接映射 INVON/INVOFF（同 CYD） */
    if (panel_handle) esp_lcd_panel_invert_color(panel_handle, en);
}

/* 默认 mirror(true,true)（沿用 CYD / esp32-ILI9341 实测值，esp_lcd 标准语义）。
...
void bsp_init(void)
{
    ...
    /* 传输完成信号量：bsp_lcd_push / fade_out 等待 DMA 完成用 */
    ...
    esp_lcd_panel_dev_config_t panel_cfg = {
        ...
        .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_BGR,   /* 面板 BGR 原生（同 CYD 实测） */
        .bits_per_pixel = 16,
    };
    ESP_ERROR_CHECK(esp_lcd_new_panel_ili9341(io_handle, &panel_cfg, &panel_handle));
    ESP_ERROR_CHECK(esp_lcd_panel_reset(panel_handle));
    ESP_ERROR_CHECK(esp_lcd_panel_init(panel_handle));
    /* ILI9341 默认 INVOFF 即正常颜色，无需强制 INVON；反色是运行时开关 */
    ESP_ERROR_CHECK(esp_lcd_panel_invert_color(panel_handle, false));
    ESP_ERROR_CHECK(esp_lcd_panel_set_gap(panel_handle, 0, 0));
    /* 横屏 320x240，mirror 基线沿用 CYD / esp32-ILI9341 实测组合 */
    ...
    ESP_LOGI(TAG, "BSP ready (esp32s3-ILI9341-320_240-xpt2046-ec11, ILI9341 %dx%d)",
             LCD_H_RES, LCD_V_RES);
}
...
#endif /* CONFIG_BOARD_ESP32S3_ILI9341_EC11 */
