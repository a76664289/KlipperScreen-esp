/*
 * BSP: esp32c3-st7789-320_240-ec11 —— ESP32-C3 无触摸旋钮系统：
 * 240x320 ST7789 SPI 屏（横屏 320x240）+ EC11 旋转编码器，无触摸。
 *
 * 目标板（同一接线、同一固件）：
 *   - 合宙 CORE ESP32-C3（USB 直连版，非 CH340 版）
 *   - ESP32-C3 Super Mini（排针只引出 GPIO0~10/20/21，本接线全部落在 0~10）
 *
 * 引脚（避开 8=Super Mini 板载 LED、12/13=合宙板 LED、18/19=USB、20/21=UART0）：
 *   LCD 走 SPI2：SCK=2 MOSI=3 CS=7 DC=6 RST=5 BL=4
 *   EC11：A=0 B=1 SW=10（sdkconfig 里配置；C3 全部 GPIO 有内部上拉）
 *   息屏/唤醒：板载 BOOT 键 GPIO9（低电平有效）
 *
...
/* 引脚：合宙 CORE / Super Mini 两板通用（见文件头注释） */
...
#define PIN_LCD_BL         4
#define PIN_BTN_BOOT       9   /* 板载 BOOT 键：按下息屏，再按唤醒 */

#define LCD_H_RES          320   /* 横屏逻辑分辨率（面板原生 240x320，swap_xy） */
#define LCD_V_RES          240
#define LCD_SPI_HZ         (40 * 1000 * 1000)
#define DRAW_BUF_LINES     40    /* 2×320×40×2=51.2KB，C3 无 PSRAM 也放得下 */

/* ST7789 模组差异调参：方向不对改镜像，边缘有彩边/偏移改 gap */
...
{
    /* ST7789 默认 INVOFF 即正常颜色，"反色"开关直接映射 INVON/INVOFF */
...
void bsp_init(void)
{
    ...
    /* 无指针输入：本机完全由 EC11 旋钮导航（bsp_input_init 按 sdkconfig 创建；
       C3 无 PCNT，编码器自动走 GPIO 中断软件解码后端） */
    ...
    /* C3 单核：不绑核，由调度器自由安排 */
    BaseType_t task_ok = xTaskCreate(lvgl_task, "lvgl", 12288, NULL, 4, NULL);
    ESP_ERROR_CHECK(task_ok == pdPASS ? ESP_OK : ESP_ERR_NO_MEM);

    ESP_LOGI(TAG, "BSP ready (esp32c3-st7789-320_240-ec11, ST7789 %dx%d)",
             LCD_H_RES, LCD_V_RES);
}
...
#endif /* CONFIG_BOARD_ESP32C3_ST7789_EC11 */
