/*
 * BSP: esp32-ST7796-320_240-ec11 —— CYD 同款主控（ESP32）的无触摸中屏旋钮系统：
 * 240x320 ST7796 SPI 屏（横屏 320x240）+ EC11 旋转编码器，无触摸。
 *
 * 引脚、EC11 与息屏按钮配置全部复用 esp32-ILI9341-320_240-ec11（对齐 CYD 2432S028R）：
 ...
 *
 * 仅显示控制器由 ILI9341 换成 ST7796：驱动/颜色格式/反色语义沿用 E32R35T 与
 * esp32s3-st7796-480_320-xpt2046-ec11 的实测实现（BGR、INVOFF 即正常颜色、
 * 横屏 swap_xy + mirror(true,true)）。
 * 注意：ST7796 模组在 320x240 窗口下的 MADCTL 方向与窗口偏移（gap）可能随
 * 模组不同而不同——若实机画面镜像/颠倒或边缘有彩边/偏移，调下面的
 * LCD_MIRROR_X/Y 与 LCD_GAP_X/Y（当前初始值沿用 480x320 模组的横屏语义，需实机确认）。
 */
#include "sdkconfig.h"
#if CONFIG_BOARD_ESP32_ST7796_EC11
...
/* 引脚：与 esp32-ILI9341-320_240-ec11 一致（对齐 CYD 2432S028R） */
...
#define LCD_H_RES          320   /* 横屏逻辑分辨率（面板原生 240x320，swap_xy） */
#define LCD_V_RES          240
#define LCD_SPI_HZ         (40 * 1000 * 1000)
#define DRAW_BUF_LINES     40    /* 2×320×40×2=51.2KB DMA，CYD 同规模已验证 */

/* ST7796 模组差异调参（320x240 窗口，需实机确认）：
 *   初始值沿用 E32R35T / esp32s3-st7796-480_320-xpt2046-ec11 的横屏语义
 *   （BGR、INVOFF 正常色、swap_xy + mirror(true,true)，gap 0）；
 *   方向不对改 LCD_MIRROR_X/Y，边缘有彩边/偏移改 LCD_GAP_X/Y。 */
...
{
    /* ST7796 默认 INVOFF 即正常颜色（E32R35T 实测，开了呈底片），
       "反色"开关直接映射 INVON/INVOFF */
...
void bsp_init(void)
{
    ...
    esp_lcd_panel_dev_config_t panel_config = {
        ...
        .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_BGR,   /* 面板 BGR 原生（同 E32R35T 实测） */
        .bits_per_pixel = 16,
    };
    ESP_ERROR_CHECK(esp_lcd_new_panel_st7796(io, &panel_config,
                                             &panel_handle));
    ESP_ERROR_CHECK(esp_lcd_panel_reset(panel_handle));
    ESP_ERROR_CHECK(esp_lcd_panel_init(panel_handle));
    /* ST7796 默认 INVOFF 即正常颜色（E32R35T 实测，INVON 呈底片） */
    ...
    ESP_LOGI(TAG, "BSP ready (esp32-ST7796-320_240-ec11, ST7796 %dx%d)",
             LCD_H_RES, LCD_V_RES);
}
...
#endif /* CONFIG_BOARD_ESP32_ST7796_EC11 */
