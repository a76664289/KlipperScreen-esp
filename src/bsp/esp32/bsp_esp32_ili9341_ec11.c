/*
 * BSP: esp32-ILI9341-320_240-ec11 —— CYD 同款主控（ESP32）的无触摸中屏旋钮系统：
 * 240x320 ILI9341 SPI 屏（横屏 320x240）+ EC11 旋转编码器，无触摸。
 *
 * 引脚、EC11 与息屏按钮配置全部复用 esp32-st7789-320_240-ec11（对齐 CYD 2432S028R）：
 ...
 *
 * 仅显示控制器由 ST7789 换成 ILI9341：面板参数沿用 CYD 实测值
 * （BGR、横屏 mirror(true,true)，ILI9341 默认 INVOFF 即正常颜色）。
 * 若手上的模组方向/边缘有偏移，调下面的 LCD_MIRROR_X/Y 与 LCD_GAP_X/Y。
 */
#include "sdkconfig.h"
#if CONFIG_BOARD_ESP32_ILI9341_EC11
...
/* 引脚：与 esp32-st7789-320_240-ec11 一致（对齐 CYD 2432S028R） */
...
/* ILI9341 模组差异调参：方向不对改镜像，边缘有彩边/偏移改 gap（基线同 CYD） */
...
void bsp_init(void)
{
    ...
    /* 横屏 320x240；swap_xy 后屏幕水平轴对应面板 Y 轴，水平镜像要翻 mirror_y（同 CYD） */
    ESP_ERROR_CHECK(esp_lcd_panel_swap_xy(panel_handle, true));
    ESP_ERROR_CHECK(esp_lcd_panel_mirror(panel_handle, LCD_MIRROR_X,
                                         LCD_MIRROR_Y));
    /* ILI9341 默认 INVOFF 即正常颜色，无需强制 INVON */
    ...
    ESP_LOGI(TAG, "BSP ready (esp32-ILI9341-320_240-ec11, ILI9341 %dx%d)",
             LCD_H_RES, LCD_V_RES);
}
...
#endif /* CONFIG_BOARD_ESP32_ILI9341_EC11 */
