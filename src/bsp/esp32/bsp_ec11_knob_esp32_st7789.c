/*
 * BSP: esp32-st7789-320_240-ec11 —— CYD 同款主控（ESP32）的无触摸中屏旋钮系统：
 * 240x320 ST7789 SPI 屏（横屏 320x240）+ EC11 旋转编码器，无触摸。
 *
 * 引脚与 esp32-st7735s-128_160-ec11 完全一致（全部对齐 CYD 2432S028R）：
 ...
 *
 * ST7789 有 ESP-IDF 官方驱动，默认 INVOFF 即正常颜色（不像 ST7735S 必须 INVON）。
 * 若手上的模组方向/边缘有偏移，调下面的 LCD_MIRROR_X/Y 与 LCD_GAP_X/Y。
 */
#include "sdkconfig.h"
#if CONFIG_BOARD_EC11_KNOB_ESP32_ST7789
...
/* 引脚：与 esp32-st7735s-128_160-ec11 一致（对齐 CYD 2432S028R） */
...
void bsp_init(void)
{
    ...
    ESP_LOGI(TAG, "BSP ready (esp32-st7789-320_240-ec11, ST7789 %dx%d)",
             LCD_H_RES, LCD_V_RES);
}
...
#endif /* CONFIG_BOARD_EC11_KNOB_ESP32_ST7789 */
